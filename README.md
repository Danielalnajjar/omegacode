...
For filtered Node tests, put filtering options before file arguments rather than forwarding them through `pnpm test --`, for example: `pnpm exec node --test --import ./test/register.mjs --test-skip-pattern='verified packed global refresh' ./test/packaging.test.ts`.
