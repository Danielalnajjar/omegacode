...
Each call resolves the source configuration at `$XDG_CONFIG_HOME/muse`, or `~/.config/muse` when unset. When source `settings.json` exists, the call creates a private `<tmp>/xdg/muse` directory (0700), writes the settings (0600) with only the top-level `mcpServers` member removed, and symlinks every other source entry. Authentication is never copied, HOME is never overridden, and only the child receives the private XDG_CONFIG_HOME. Without source settings there is no override. The private directory and prompt are removed in finally. The Muse 1.2.1-R2847.1 spike measured zero broker clients with this configuration; workspace settings, permission profiles, agents overlays, environment flags, and defaults/policy documents did not provide usable per-run MCP isolation.
...
| --- | --- |
| run.terminal.completed with terminal completed | Success; payload.text is authoritative. |
...
The exec stream reports no usage, so results use emptyUsage() and emit no usage progress. Structured output is requested in the prompt and parsed and validated locally; the runtime retains ownership of its single corrective schema attempt. There is no Muse extraction turn or retry layer. Existing provider defaults, cache identity, and keys remain unchanged.
