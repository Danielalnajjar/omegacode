...
Muse librarians: `museExecutionProfile: "workflow-research-v1"` keeps exactly `btca`, `executor_research`, `grok_search`, and `mintlify` from the user's Muse `settings.json`, re-enables them (they may be `enabled: false` for interactive Muse), and expands `${VAR}` in their `headers`/`env` from OmegaCode's environment (for example `EXECUTOR_API_KEY`), because Muse does not. A missing server or unset variable fails `invalid_config` before launch. Native Muse subagents are unavailable in OmegaCode's untrusted workspaces, so a librarian is its own `agent()` call whose Context Pack feeds later workers; `muse-librarian` (user workflow) carries the librarian prompt. The profile also opens shell network in the read-only sandbox (writes stay denied) and passes `gh auth token` as `GH_TOKEN`, because the sandbox hides gh's keychain login; a logged-out `gh` fails `invalid_config`. Read-only Claude workers accept read-only `gh` commands (`gh api` GETs and GraphQL queries, `view`/`list`/`search`), so librarians on every provider can use `gh`.
