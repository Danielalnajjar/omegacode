...
- **muse** (Muse CLI ≥ 1.2.1) — spawn-per-call `muse exec --json`. Supports `read-only` and `danger-full-access`, not `workspace-write`; effort passes through unchanged and usage is unreported. Override the binary with `MUSE_BIN`. See the Muse section below.
...
Muse (`provider: "muse"`, CLI ≥ 1.2.1) supports read-only and danger-full-access only; workspace-write fails before launch. Efforts pass through unchanged. Usage is unreported. Each attempt uses private settings without MCP servers and symlinked login state, never copied credentials.
