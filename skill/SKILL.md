...
- **fx** (exact CLI `v0.0.6` only, digest-admitted) — spawn-per-call `fx ask --json --no-save` with the prompt on stdin. Requires `FX_BIN` as an absolute path to the admitted binary and `OMEGACODE_FX_HOME` as an explicit absolute private managed automation HOME (never inherited `~/.fx`). **Requires `sandbox: "danger-full-access"`** and `approval: "never"`. `effort`, `maxTurns`, and `serviceTier` are rejected. Usage is unavailable: returned zeros are a known lower bound marked `incomplete`; a finite run `--budget` is rejected before spawn. Structured output is validated locally; a miss uses OmegaCode's one corrective full turn because v0.0.6 has no enforceable no-tools mode.
...
codex, claude-code, and grok honor every `sandbox` mode (read-only by default); **opencode, pi, and fx are full-access-only** — every call on them needs an explicit `sandbox: "danger-full-access"` (per call or via `--sandbox`), a deliberate fail-closed choice because those CLIs cannot honestly enforce confinement. All providers honor `cwd`; `worktree: true` isolates parallel file edits for providers that support `workspace-write`, and is rejected for fx. Outdated opencode/pi/grok binaries are refused at runtime (`provider_outdated`); fx admits only exact `v0.0.6` plus the platform digest. `omegacode doctor` flags them up front.
...
omegacode run <file.workflow.js | name> [--args '<json>' | --args-file <f>]
                                       [--provider codex|claude-code|opencode|pi|grok|fx --model m] [--effort e]
