...
- **amp** (Amp) — in-process backend that runs **only under the Amp omegacode plugin**: the `omegacode_run_workflow` tool sets `OMEGACODE_AMP_SOCKET`, and without that socket every amp call fails immediately. `model` is an Amp `provider/model` string (e.g. `"xai/grok-4.5"`, `"openai/gpt-5.6-sol"`), required whenever `provider: "amp"` is set (both-or-neither). Each `agent()` becomes a child Amp thread of the calling thread — it shows up, and is billed, in the Amp dashboard. **Requires `sandbox: "danger-full-access"`** — Amp threads get the full Amp tool set and no OS confinement, so `read-only`/`workspace-write` are rejected; `maxTurns` and any approval other than `never` are rejected too. Amp exposes no token usage to plugins, so amp turns report **zero** usage: `budget.spent()` under-counts them and a `--budget` ceiling never stops an amp fan-out — read the Amp dashboard for real cost. Structured output via a silent extraction turn.
...
codex and claude-code honor every `sandbox` mode (read-only by default); **opencode, pi, and amp are full-access-only** — every call on them needs an explicit `sandbox: "danger-full-access"` (per call or via `--sandbox`), a deliberate fail-closed choice because none of those three can honestly enforce confinement. All providers honor `cwd`; `worktree: true` isolates parallel file edits regardless of provider. Outdated opencode/pi binaries are refused at runtime (`provider_outdated`); `omegacode doctor` flags them up front.
...
omegacode run <file.workflow.js | name> [--args '<json>' | --args-file <f>]
                                       [--provider codex|claude-code|opencode|pi|amp --model m] [--effort e]
