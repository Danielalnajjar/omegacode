...
test("L6: usageFromResult sums cache read/creation tokens into inputTokens", () => {
  ...
  assert.equal(u.cacheReadInputTokens, 4000)
  assert.equal(u.cacheCreationInputTokens, 500)
})
...
test("L6: usageFromResult tolerates missing/malformed usage", () => {
  ...
  assert.equal(u.cacheReadInputTokens, undefined)
  assert.equal(u.cacheCreationInputTokens, undefined)
